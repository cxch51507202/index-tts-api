...
# Windows/Gradio ?????????? IndexTTS ????????????
# ????/??? ASCII ????????????????????????????????
def normalize_prompt_audio(prompt_path, max_seconds=12.0):
    if not prompt_path:
        return prompt_path
    import hashlib
    import wave
    src = os.fspath(prompt_path)
    safe_dir = os.path.join("outputs", "prompt_cache")
    os.makedirs(safe_dir, exist_ok=True)
    digest = hashlib.md5((src + str(os.path.getmtime(src))).encode("utf-8", "ignore")).hexdigest()[:12]
    dst = os.path.abspath(os.path.join(safe_dir, f"prompt_{digest}_{int(max_seconds)}s.wav"))
    ...
    try:
        with wave.open(src, "rb") as r:
            params = r.getparams()
            fr = r.getframerate()
            max_frames = int(float(max_seconds) * fr)
            frames = r.readframes(min(r.getnframes(), max_frames))
        with wave.open(dst, "wb") as w:
            w.setparams(params)
            w.writeframes(frames)
        return dst
    except Exception as e:
        print(f">> prompt audio normalize skipped: {e}")
        return src
...
def gen_single(prompt, text, infer_mode, max_text_tokens_per_sentence=120, sentences_bucket_max_size=4,
    ...
    prompt = normalize_prompt_audio(prompt, max_seconds=12.0)
    print(f">> using prompt audio: {prompt}")
    # set gradio progress
    ...
    mode_text = str(infer_mode or "")
    is_batch = ("?" in mode_text) or ("fast" in mode_text.lower()) or ("batch" in mode_text.lower())
    if not is_batch:
